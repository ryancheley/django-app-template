...
- [X] T043 [US3] Coverage floor, measurement two of two (linked to T032): run the instantiation dry-run on a repo copy, measure coverage there, set the final `--cov-fail-under` in `pyproject.toml` to the lower of the two measurements per research R5 (update the in-file comment with both numbers). Serial (pyproject).
...
- [X] T044 [US3] **CHECKPOINT — Phase 4 closes.** Scripted dry run, all commands exiting 0:
